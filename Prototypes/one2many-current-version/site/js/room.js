// Initialisation gÈolocalisation
var lat;
var lon;
var check = false;
...
function errorCallback(error) {
    switch (error.code) {
        case error.PERMISSION_DENIED:
            alert("L'utilisateur n'a pas autorisÈ l'accËs ‡ sa position");
            break;
        case error.POSITION_UNAVAILABLE:
            alert("L'emplacement de l'utilisateur n'a pas pu Ítre dÈterminÈ");
            break;
        case error.TIMEOUT:
            alert("Le service n'a pas rÈpondu ‡ temps");
            break;
    }
}
;
...
// Timer pour avoir le temsp de rÈcupÈrer la position 
sleep(10000);
...
// On cr√©e l'√©v√©nement recupererParticipants pour r√©cup√©rer directement les participants sur le serveur
socket.on('recupererParticipants', function(participants) {
  //r√©initialisation de la liste des participants au niveau graphique lors des √©ventuelles m√†j de cette derni√®re
  $('#list_parts').children('li').remove();
  // participants est le tableau contenant tous les participants qui ont √©t√© √©cris sur le serveur
